<?php
//DAVID COHEN
//REWRITE of block_schedulenex.php 1/19/11 due to schema changes
...
//separates it by "and" if there are only two djs
	...
	if($day == date('w',strtotime('now')))
		$start_time = date("g:i a", strtotime("$start_hour:$start_min"));
	else
		$start_time = date('D', strtotime('tomorrow')) . " " . date("g:i a", strtotime("$start_hour:$start_min"));
